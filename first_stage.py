...
def first_stage():
    ...
    land_obs_2 = setup.Obstacle(py.image.load("image\장애물_지상.png"),setup.screen_width*1.3)
    ...
    while running:
        ...
        if Puang.is_jumping:
            ...
            Puang.posY -= speed
            if Puang.posY < ground - screen_height * 0.17:  # 점프 높이
        ...
        for i in range(0,len(obs_list)):
            ...
            if Puang.posX  < (obs_list[i].posX + obs_list[i].width)- obs_list[i].width*0.1 and Puang.posX + Puang.width > obs_list[i].posX + obs_list[i].width*0.1:
                if Puang.posY + Puang.height > obs_list[i].posY + obs_list[i].height*0.1:
